...
func ffightuBoard() *Board {
	ffightu := Board{}

	ffightu.Cpsb = 4

	ffightu.Z80.Size = 65_536
	ffightu.Z80.Epromer = ffightuZ80EPromer

	ffightu.M68k.Size = 1_048_576
	ffightu.M68k.Epromer = ffightuM68kEPromer

	ffightu.GFX.Size = 4 * 0x80000 // 0x200000 = 2 MiB
	ffightu.GFX.Epromer = ffightuGFXEpromer
	ffightu.GFX.Roms = []ROM{
	...
	ffightu.Oki.Size = 0x40000
	ffightu.Oki.Epromer = ffightuOkiEpromer
	ffightu.Oki.Roms = []ROM{
	...
	// RGBA
	ffightu.Post.PostPalette = color.Palette{
	...
	ffightu.Post.PostTile = 4
	ffightu.Post.PostTileWidth = 3
	ffightu.Post.PostTileHeight = 5

	return &ffightu
}

func ffightuGFXEpromer(gfxrom []byte, outDir string) {
	if gfxrom == nil {
		return
	}
	// Split it
...
func ffightuZ80EPromer(rom []byte, outputDir string) {
...
func ffightuM68kEPromer(r []byte, outputDir string) {
	...
	writeToFile(r[1*ROM_SIZE+1:], 1, 2, ROM_SIZE, outputDir+"ffu_43.12h")
    // WORD SWAPPED ROM ??
	writeToFile(r[2*ROM_SIZE:], 1, 2, 2*ROM_SIZE, outputDir+"ff-32m.8h")
}

func ffightuOkiEpromer(rom []byte, outDir string) {
	if rom == nil {
		return
	}
